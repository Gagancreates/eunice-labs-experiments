cat > eval.py << 'EOF'
  ...
  base = AutoModelForCausalLM.from_pretrained("deepseek-ai/DeepSeek-R1-Distill-Qwen-7B", quantization_config=bnb_config, device_map="auto")
  ...
  for label, samples in [("easy", easy), ("medium", medium)]:
      ...
      results[label] = {"mean_think_tokens": round(sum(think_lens)/len(think_lens), 1), "max_think_tokens": max(think_lens), "samples_with_think": sum(1 for
   t in think_lens if t > 0)}
